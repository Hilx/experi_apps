...
void graph_printout(ptr_t ptr2graph){
	...
	printf("Number of Keys = %lu, ptr2hashtable = %x\n", graph_root.key_cnt, graph_root.ptr_hashtable);
	for(int i = 0; i < HASHTABLE_NUM_BUCKETS; i++){
		...
		while(ptr2bucket != NULL_PTR){
			...
			printf("[%lu] (%lu links) -> [", bucket_node.key, bucket_node.link_cnt);
