...
void graph_addedge(data_t * m2mem, data_t * m2alloc, data_t * buffer, ptr_t ptr_root, data_t key0, data_t key1){
	if(key0 != key1){ // a node cannot has a link to itself
		...
		if(graph_root.key_cnt == 0){ // NULL GRAPH
			...
			hashtable_update(m2mem, m2alloc, buffer, ptr_hashtable, hash_id0, ptr_bucket_node0);
			hashtable_update(m2mem, m2alloc, buffer, ptr_hashtable, hash_id1, ptr_bucket_node1);
			...
			if(ptr_bucket == NULL_PTR){
				...
				hashtable_update(m2mem, m2alloc, buffer, graph_root.ptr_hashtable, hash_id, ptr_bucket_local);
...
void second_key_process(data_t * m2mem, data_t * m2alloc, data_t * buffer, ptr_t ptr_root, struct root_t graph_root, data_t key1, data_t linking_key){
	...
	if(ptr_bucket == NULL_PTR){
		...
		if(bucket_node_pair.bucket_node_curr.key == key1){ // key1 exists
			...
			ptr_t ptr_keyset_local = create_link(m2mem, m2alloc, buffer, ucket_node_pair.bucket_node_curr.ptr_keyset, keyset_node_pair, linking_key);				
			struct bucket_node_t bucket_node2update;
			bucket_node2update.ptr_next = bucket_node_pair.bucket_node_curr.ptr_next;
			bucket_node2update.ptr_keyset = ptr_keyset_local;
			bucket_node2update.link_cnt = bucket_node_pair.bucket_node_curr.link_cnt + 1;
...
data_t graph_printout(data_t * m2mem, data_t * m2alloc, data_t * buffer, ptr_t ptr2graph){
	...
	for(int i = 0; i < HASHTABLE_NUM_BUCKETS; i++){
		...
		while(ptr2bucket != NULL_PTR){
			...
			for(int j = 0; j < bucket_node.link_cnt; j++){
